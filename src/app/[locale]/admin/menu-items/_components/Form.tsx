...
function Form({translations,categories,product}:{translations:Translations,categories:Category[],product:ProductWithRelations}) {
...
const UploadImage = ({selectedImage ,setSelectedImage}:{selectedImage:string,setSelectedImage: React.Dispatch<React.SetStateAction<string>>}) => {
    ...
    <div className="group mx-auto md:mx-0 relative w-[200px] h-[200px] overflow-hidden rounded-full">
        {selectedImage && (
        <Image src={selectedImage} alt={"Add Product Image"} width={200} height={200} className="rounded-full object-cover"/>
        )}
        <div
        className={`${
          selectedImage
    ...
    >
      <AccordionItem value="item-1" className="border-none">
        ...
        <AccordionContent className="flex flex-col gap-4 text-balance">
          <ItemOptions optionKey={ItemOptionsKeys.SIZES} translations = {translations} state={sizes} setState={setSizes} />
        </AccordionContent>
      </AccordionItem> 
    </Accordion></>
};
  ...
  return <>
    ...
    >
      <AccordionItem value="item-1" className="border-none">
        ...
        <AccordionContent className="flex flex-col gap-4 text-balance">
          <ItemOptions optionKey={ItemOptionsKeys.EXTRAS} translations = {translations} state={extras} setState={setExtras} />
        </AccordionContent>
      </AccordionItem> 
    </Accordion></>
};

const FormActions = ({translations,pending ,product,}:{translations:Translations;pending:boolean; product?: ProductWithRelations;}) =>{
